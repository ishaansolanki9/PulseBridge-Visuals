...
- Device capture failures enter a reconnect loop. On Windows, the Rekordbox route follows the process tree to its Windows audio-session endpoint; automatic output capture checks that match first and then the default/remaining endpoints, while a user-selected endpoint stays fixed. Process-specific activation is absent from normal code because native termination cannot be caught or recovered in-process. Initialized/silent remains distinct from samples flowing.
...
- Overall lifecycle, renderer state, audio route/sample flow, phrase provenance, process detection, and Windows audio-session detection are independent status fields. A found process never means samples or phrase metadata are connected.
