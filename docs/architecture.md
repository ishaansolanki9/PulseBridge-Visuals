...
Renderer time is monotonic seconds since performance start, not frame count. User parameters and musical parameters use asymmetric attack/release envelopes. A dedicated audio-drive envelope converts energy and spectral events into a proportional 0–1 motion/color dial. The `SceneDirector` keeps macro identity stable long enough to read, chooses from phrase-specific subsets of the 26 illusions using the live structure signature and phrase index, emits one family except for normalized old/new crossfades, and schedules at most two compatible modifier envelopes at musical events. There is no wall-clock scene shuffle.

Native JSON-line logs rotate in the platform log directory. A synchronously replaced `last-session.json` records the active report and last risky stage. An unfinished marker becomes `previous-run-report.json` on the next launch. Runtime status and reports separate process detection, Windows audio-session detection, capture initialization, packet receipt, non-silent signal, reactive readiness, route, phrase provenance, lifecycle, and renderer state.
