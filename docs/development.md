...
Open Rekordbox before PulseBridge so `process:auto` can discover its deterministic process tree. That source is strict process capture: it never falls back to a physical output and therefore never admits unrelated application audio. For the DDJ-1000 topology, leave the controller as Rekordbox's primary ASIO device and enable PC MASTER OUT to make a Windows render stream owned by Rekordbox. The separate `output:auto` source is intentionally labeled all-app audio and probes active Windows render endpoints. Detection, client initialization, packet arrival, non-silent signal, route, and phrase provenance remain separate.

The Microsoft process-loopback API requires Windows 10 build 20348 or later. Its activation parameters, operation, and agile completion handler stay alive until Windows invokes the callback instead of being dropped by an unsafe application timeout. The API has no cancellation operation, so its lifecycle must not regain a finite application-side timeout. Validate Audio-only, Renderer-only, Full startup, Rekordbox restart, rapid retry, and late-callback behavior on real Windows hardware. Native Windows work should also be checked with the same commands used by `scripts/build-windows.ps1`.
