...
Auto direction is native because it depends on phrase/musical state and a timed random shuffle. Production family IDs 0–27 are declared by `VisualFamily` and dispatched by the WGSL `visual_family` switch; all 28 must remain distinct and available to Auto. Modifier IDs 0–7 are Palette Drift, Beat Zoom, Bass Warp, High Sparkle, Feedback Trails (`EchoTrails` internally), Mirror Fold, Chromatic Split, and Impact Bloom. White flashes default to Off; energetic motion must remain legible with flashes disabled.

Whenever WGSL visual math changes, preserve the preview's palette and luminance behavior, but keep the browser preview deliberately cheaper than the production library. The WGSL validator test and the 28-distinct-family test are mandatory.
