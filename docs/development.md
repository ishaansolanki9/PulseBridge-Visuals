...
Auto direction is native because it depends on phrase/musical state and a timed shuffle. Production family IDs 0–11 are declared by `VisualFamily` and dispatched by the WGSL `visual_family` switch; all 12 must remain distinct, abstract, and available to Auto. IDs 0–3 are the recurring core: Color-Splotch Wave, Multi-Layer Wave Field, Fractal Bloom, and Recursive Tunnel. The bounded modifier set is Palette Drift, Beat Zoom, Feedback Trails (`EchoTrails` internally), and Impact Bloom. White flashes default to Off; energetic motion must remain legible with flashes disabled.

Whenever WGSL visual math changes, preserve the preview's palette and luminance behavior, but keep the browser preview deliberately cheaper than the production library. The WGSL validator test and the 12-distinct-preset/core-rotation tests are mandatory. Each family should retain a recognizable structure in a frozen frame, and the automatic full-library slots must keep every family reachable.
