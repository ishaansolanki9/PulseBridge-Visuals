...
Open Rekordbox before PulseBridge so `process:auto` can report detection. The normal Windows build deliberately uses stable output loopback and automatically probes the default render endpoint followed by the other active endpoints until non-silent samples arrive. The affected laptop repeatedly raised `STATUS_HEAP_CORRUPTION` while `ActivateAudioInterfaceAsync` was pending; explicit Windows output capture completed successfully on the same GPU and audio hardware. Detection, client initialization, packet arrival, non-silent signal, route, and phrase provenance remain separate.
