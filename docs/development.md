...
Open Rekordbox before PulseBridge and enable PC MASTER OUT when using the DDJ-1000 ASIO topology so Rekordbox creates a capturable Windows render stream. The `output:auto` source is the safe Windows default: it probes all active render endpoints and is intentionally labeled all-app audio. Persisted `process:auto` settings migrate to `output:auto`; the process source is disabled and must not call `ActivateAudioInterfaceAsync`. Detection, client initialization, packet arrival, non-silent signal, route, and phrase provenance remain separate.
