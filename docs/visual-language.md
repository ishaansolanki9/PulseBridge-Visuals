...
An original oscilloscope-inspired spectral signal ribbon is drawn only during Spinning Alien and, more faintly, Watching Eye. It is not an imported MilkDrop preset and does not require Winamp, SHOUTcast, projectM, Butterchurn, an API key, or external visual assets. Keeping it contextual prevents a line overlay from obscuring unrelated scenes.
