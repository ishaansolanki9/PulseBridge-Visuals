...
## Windows native pipeline timeout (September 11–12, 2026)

- New supplied log: the latest Windows session reaches settings, display, Intel Arc/DX12 adapter selection, device creation, and WGSL validation successfully. Five Start attempts between 01:30 and 01:32 UTC stop at `renderer.pipeline.begin`; none reaches pipeline completion or first-present. Each hits the 12-second startup deadline and then a renderer join timeout. The separate previous-run report describes an older unexpected exit after settings loaded; it does not diagnose these newer failures.
- Target: Windows build 26200.9445, Intel Arc integrated graphics driver 32.0.101.6881, 3840×2400 display. The older pre-expansion session completed pipeline creation in 5.76 seconds. The log proves the native pipeline stall, but does not distinguish CPU shader compilation from the driver's pipeline call.
- The Windows build previously used wgpu's Auto compiler choice without shipping DXC, allowing fallback to slow legacy FXC. Enable wgpu `static-dxc` and explicitly select `StaticDxc` on supported Windows MSVC targets in `visuals/gpu.rs`. This ships the compiler in the executable for both development and installer builds. Live rendering, diagnostics, and auditions share the instance policy and fullscreen pipeline creation. All scene shaders and visual math remain unchanged.
- A renderer lease stays with a worker through native calls and cleanup. A startup/diagnostic timeout cannot free it early: further attempts return `GPU_RENDERER_BUSY` until the worker exits instead of spawning additional stuck GPU workers. Check cancellation between pipeline compilations. Durable events now identify the selected compiler and fullscreen/line/compositor stages; `GPU_STARTUP_TIMEOUT` includes the latest renderer progress and adapter.
- Audio is an independent condition: the latest attempts initialized Realtek shared-mode loopback and received packets, but no non-silent signal. Recovering-state messages now retain PC MASTER OUT guidance. When using the DDJ-1000 ASIO route, enable PC MASTER OUT in Rekordbox Performance mode and play a track. Do not reintroduce process-loopback activation.
- Added `native_pipeline_startup_audition`: compiles the production fullscreen, line, compositor, and presentation pipelines within the unchanged 12-second budget, then checks non-black GPU readback for representative 2D, Tron, 3D, and a crossfade. Windows CI runs it explicitly with a two-minute process deadline; this is stronger than WGSL parsing alone. Local Apple M1/Metal startup measured 0.793 seconds. Intel Arc and live Rekordbox retesting is still required; the local Mac measurement does not validate the Windows driver.
- Local validation: frontend lint/typecheck/build, all nine browser regressions, 87 native unit tests (nine GPU auditions ignored in the default suite), strict native Clippy, Windows cross-target checking, and the explicitly run startup GPU audition passed. Unit tests also cover refusing five retries while a GPU worker is still alive, releasing ownership on panic, and cancellation preserving the last startup stage. The installed Mac application has not been replaced by this source fix.
