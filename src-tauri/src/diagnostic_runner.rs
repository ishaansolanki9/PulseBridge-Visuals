...
const FIRST_PACKET_TIMEOUT: Duration = Duration::from_secs(4);
...
fn run_audio_probe(
    ...
    if init_snapshot.fallback_attempted {
        ...
        let preferred_code = if preferred_failure.contains("WINDOWS_BUILD_UNSUPPORTED") {
            "WINDOWS_BUILD_UNSUPPORTED"
        } else if preferred_failure.contains("PROCESS_LOOPBACK_ACTIVATION_FAILED") {
            "PROCESS_LOOPBACK_ACTIVATION_FAILED"
        } else if preferred_failure.contains("Rekordbox is not running") {
            "REKORDBOX_PROCESS_NOT_FOUND"
        } else {
            "PROCESS_LOOPBACK_ACTIVATION_FAILED"
        };
        ...
        if report.verdict == DiagnosticVerdict::Pass {
            report.verdict = DiagnosticVerdict::Degraded;
            report.failure_stage = Some("audio.preferredRoute".to_string());
            report.failure_code = Some(preferred_code.to_string());
        }
